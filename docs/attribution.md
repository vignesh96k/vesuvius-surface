...
| **The 1st-place team's own writeup** (a *different* team from arunodhayan — see note below) | Describes their post-processing chain: remove small components → per-sheet closing → height-map gap patching → hole plugging → global fill_holes. Also states plainly they "did not find an effective solution to the problem of touching sheets" and relied on nnU-Net itself to minimize it, with no postprocessing fix of their own. | Chain reimplemented independently in `src/vesuvius_surface/postprocess/first_place.py`, following the writeup's description and the mechanics detail from a public reimplementation (`bshepp/volumen`, credited in-module) for the height-map/LUT specifics not fully spelled out in the writeup itself. Their stated touching-sheets gap is independent, external confirmation of exactly the problem `postprocess/unmerge.py` targets — not just this project's own ablation finding. |
