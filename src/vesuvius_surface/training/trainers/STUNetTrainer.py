...
class STUNetTrainer(nnUNetTrainer):
    ...
    def build_network_architecture(plans_manager,
        ...
        # Parameter order fixed to match nnUNetTrainer.initialize()'s real positional call
        # convention (plans_manager, configuration_manager, num_input_channels,
        # num_output_channels, enable_deep_supervision) -- the original upstream port had
        # dataset_json in the configuration_manager slot, a silent mismatch that would pass
        # the wrong object into the wrong parameter. num_output_channels is passed directly
        # by the caller, so no need to re-derive it via plans_manager.get_label_manager().
