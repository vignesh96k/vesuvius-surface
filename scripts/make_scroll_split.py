...
``scroll-holdout``
    One entire scroll is held out per fold. Measures generalisation to an
    unseen scroll, which is the honest but pessimistic reading. Small scrolls
    (44430 has 16 volumes, 53997 has 13) make some folds tiny.

    python scripts/make_scroll_split.py --mode stratified --n-splits 5
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--mode", choices=["stratified", "scroll-holdout"], default="stratified")
...
def print_distribution(
    ...
    if empty:
        print("\nWARNING: scrolls absent from some validation folds:")
...
def main() -> int:
    ...
    scroll_map = load_scroll_groups(args.dataset_dir)
    if not scroll_map:
        print(
            f"ERROR: no scroll_groups.json under {args.dataset_dir}. "
    ...
    print(f"\nwrote      : {out}")
    print(
        "\nnnU-Net reads this file instead of generating its own, so any model\n"
        "trained now has a recorded, verifiable split. Keep it in version\n"
        "control alongside reported numbers."
    )
    return 0
